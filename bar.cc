...
Bar::Bar(SDL_Renderer& renderer,
    ...
    point.reset(SDL_CreateTexture(&renderer,
                                  SDL_PIXELFORMAT_RGBA8888,
                                  SDL_TEXTUREACCESS_TARGET,
                                  3,
                                  3));
...
void Bar::setSpan(std::chrono::time_point<std::chrono::high_resolution_clock> s,
...
bool Bar::beat(int scale, std::chrono::time_point<std::chrono::high_resolution_clock> beatTime) {
    if (beatTime > end) {
        printf("beat off bar? %lld %lld\n", beatTime.time_since_epoch(), end.time_since_epoch());
        return true;
