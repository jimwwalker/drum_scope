...
BarDrawer::BarDrawer(int screenX, int screenY, int barX, int barY, int beats,
                     int bpm)
    : list(beats, bpm) {
  ...
  window.reset(SDL_CreateWindow("Jim's Drum Scope", SDL_WINDOWPOS_UNDEFINED,
                                SDL_WINDOWPOS_UNDEFINED, screenX + (border * 2),
                                screenY + (border * 2), SDL_WINDOW_SHOWN));
  ...
  for (int i = 0; i < barY; i++) {
    ...
    for (int j = 0; j < barX; j++) {
      int posX = j * barWidth;
      list.addBar(*renderer,
                  {posX + border, posY + border, barWidth, barHeight}, beats,
                  bpm);
    }
  }
  notified = false;
...
void BarDrawer::beat(
    int midiId, std::chrono::time_point<std::chrono::high_resolution_clock> t) {
...
void BarDrawer::run() {
  ...
  while (true) {
    ...
    } else if (currentBar != list.end() &&
               currentBar->pulse(std::chrono::high_resolution_clock::now())) {
      currentBar++;
      if (currentBar == list.end()) {
        beat1 = list.setEpoch(beat1);
        currentBar = list.begin();
